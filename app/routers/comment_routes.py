...
async def get_comments(post_id: int):
    """댓글 목록 조회 API"""
    data = comment_controller.get_comments_controller(post_id)
...
async def update_comment(post_id: int, comment_id: int, req: CommentUpdateReq, user_id: int = Depends(get_current_user_id)):
    """댓글 수정 API"""
    data = comment_controller.update_comment_controller(post_id, comment_id, req, user_id)
    return {"message": "update_comment_success", "data": data}


@router.delete("/posts/{post_id}/comments/{comment_id}")
async def delete_comment(post_id: int, comment_id: int, user_id: int = Depends(get_current_user_id)):
    """댓글 삭제 API"""
    data = comment_controller.delete_comment_controller(post_id, comment_id, user_id)
